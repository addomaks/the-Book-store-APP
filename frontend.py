
"""
A program that store book information i.e
Author, Title, year, ISBN

The user can:
View all records,
Search an entry,
Add entry,
Update entry,
Delete,
Close
"""

from tkinter import *
from backend import Database

database=Database("mysql.db")


def view_command():
    list1.delete(0,END)
    for row in database.view():
        list1.insert(END, row)


def search_command():
    list1.delete(0, END)
    for row in database.search(title_e1.get(), author_e2.get(), year_e3.get(), isbn_e4.get()):
        list1.insert(END, row)


def add_command():
    database.insert(title_e1.get(), author_e2.get(), year_e3.get(), isbn_e4.get())
    list1.delete(0, END)
    list1.insert(END, (title_e1.get(), author_e2.get(), year_e3.get(), isbn_e4.get()))


def get_selected_row(event):
    try:
        global selected_tuple
        index=list1.curselection()[0]
        selected_tuple=list1.get(index)
        e1.delete(0, END)
        e1.insert(END, selected_tuple[1])
        e2.delete(0, END)
        e2.insert(END, selected_tuple[2])
        e3.delete(0, END)
        e3.insert(END, selected_tuple[3])
        e4.delete(0, END)
        e4.insert(END, selected_tuple[4])
    except IndexError:
        pass


def delete_command():
    database.delete(selected_tuple[0])


def update_command():
    database.update(selected_tuple[0], title_e1.get(), author_e2.get(), year_e3.get(), isbn_e4.get())


window=Tk()
window.wm_title("theBookStore")
l1=Label(window, text="Title")
l1.grid(row=0, column=0)

l2=Label(window, text="Author")
l2.grid(row=0, column=2)

l3=Label(window, text="Year")
l3.grid(row=1, column=0)

l4=Label(window, text="ISBN")
l4.grid(row=1, column=2)

title_e1=StringVar()
e1=Entry(window, textvariable=title_e1)
e1.grid(row=0, column=1)

author_e2=StringVar()
e2=Entry(window, textvariable=author_e2)
e2.grid(row=0, column=3)

year_e3=StringVar()
e3=Entry(window, textvariable=year_e3)
e3.grid(row=1, column=1)

isbn_e4=StringVar()
e4=Entry(window, textvariable=isbn_e4)
e4.grid(row=1, column=3)

list1=Listbox(window, height=10, width=35)
list1.grid(row=2, column=0, rowspan=6, columnspan=2)

sb1=Scrollbar(window)
sb1.grid(row=2, column=2, rowspan=6)

list1.configure(yscrollcommand=sb1.set)
sb1.configure(command=list1.yview)

list1.bind('<<ListboxSelect>>', get_selected_row)

b1=Button(window, text="View All", width=20, command=view_command)
b1.grid(row=2, column=3)

b2=Button(window, text="Search", width=20, command=search_command)
b2.grid(row=3, column=3)

b3=Button(window, text="Add", width=20, command=add_command)
b3.grid(row=4, column=3)

b4=Button(window, text="Delete", width=20, command=delete_command)
b4.grid(row=5, column=3)

b5=Button(window, text="Update", width=20, command=update_command)
b5.grid(row=6, column=3)

b6=Button(window, text="Close", width=20, command=window.destroy)
b6.grid(row=7, column=3)
window.mainloop()
